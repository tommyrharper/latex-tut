...
% Withdraw tx lines
\draw[->, very thick] (LP1) to node[above] {Deposit Intent} node[below] {Token} (Mint);
\draw[->, very thick] (Mint) to node[above]{Shares} node[below]{Tokens} (Vault);
\draw[->, very thick] (LP2) to node[above] {Exec Deposit} (Claim);
...
\draw[->, very thick] (Burn) |- node[below, xshift=-14mm, yshift=0mm]{Token Profit} (Vault);
\draw[->, very thick] (Burn) |- (Return);
\draw[->, very thick] (Return) -| node[below, xshift=40mm]{Remaining Shares} (LP2);
